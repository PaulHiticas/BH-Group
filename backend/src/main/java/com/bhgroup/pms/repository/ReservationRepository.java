...
public interface ReservationRepository extends JpaRepository<Reservation, UUID>,
    ...
    @Query("""
            select r from Reservation r
            where r.status = com.bhgroup.pms.domain.ReservationStatus.CONFIRMED
              and r.checkInDate = :checkInDate
              and r.accessCode is not null
              and r.accessCodeSentAt is null
              and r.guestEmail is not null
            """)
    List<Reservation> findPendingCheckinInstructions(@Param("checkInDate") LocalDate checkInDate);
}
